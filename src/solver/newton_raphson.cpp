...
void NewtonRaphson::stamp_single_mosfet(size_t d, size_t g, size_t s, size_t b, 
    ...
    // Keep the actual real-world voltages for the Norton Equivalent subtraction later
    ...
    // Flip inputs for the N-type physics equations
    double v_gs = real_v_gs * sign; 
    double v_ds = real_v_ds * sign; 
    double v_bs = real_v_bs * sign;

    // Pre-Clamp to prevent math explosions in exp()
    ...
    // --- THE FIX ---
    // DO NOT multiply g_m or g_mb by sign! Conductance is always positive.
    i_ds *= sign; 

    // Norton Equivalent Current uses REAL voltages, not the flipped ones
    double i_eq = i_ds - (g_m * real_v_gs) - (g_ds * real_v_ds) - (g_mb * real_v_bs);

    // Final safety clamps before matrix injection
...
void NewtonRaphson::stamp_nonlinear_devices(const Circuit& circuit, const MnaMatrix& current_state, MnaMatrix& target_matrix) {
    for (const auto& comp : circuit.get_components()) {        
        ...
        // 2. Transmission Gate (T-Gate)
            ...
            // T-Gate is mathematically equivalent to two parallel MOSFETs
        ...
        // 3. BJT Physics Engine (Ebers-Moll / Gummel-Poon base)
...
bool NewtonRaphson::solve_dc(const Circuit& circuit, MnaMatrix& final_matrix) {
    ...
    // Warm start: Give the nodes a tiny non-zero voltage to avoid exact 0V dead-zones
    size_t num_nodes = circuit.num_nodes();
    for(size_t i = 0; i < num_nodes; ++i) current_state.x(i) = 0.01;

    double damping = 0.3; // Slower, more deliberate steps
    ...
    for (size_t iter = 0; iter < max_iterations; ++iter) {
        ...
        // 1. Update Node Voltages (Indices 0 to num_nodes - 1)
        for (size_t i = 0; i < num_nodes; ++i) {
            double raw_step = next_state.x(i) - current_state.x(i);
            double damped_step = raw_step * damping;
            
            // Limit maximum voltage change per iteration to a smooth 0.1V
            damped_step = std::clamp(damped_step, -0.1, 0.1);
            double updated_val = current_state.x(i) + damped_step;
            
            // --- PHYSICAL RAIL CLAMP ---
            // A passive/CMOS node cannot physically exceed its power rails by much.
            updated_val = std::clamp(updated_val, -1.3, 1.3);
            
            next_state.x(i) = updated_val; 
            max_delta = std::max(max_delta, std::abs(raw_step));
        }
        
        // 2. Update Source Currents (Indices num_nodes to end)
        // Currents are not bounded to +/- 1.3, so they update normally
    ...
    // std::cerr << "[libasic] Warning: Max iterations reached.\n"; // Optional mute
    final_matrix = current_state; // Return best effort even if it didn't strictly converge
    return true; // We accept the closest converged state
}
} // namespace asic